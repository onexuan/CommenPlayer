                    ...
                    switch (event.type) {
                        case MediaPlayer.Event.Buffering:
                            ...
                            if (event.getBuffering() >= 100.0f) {
                                ...
                                MLog.d("onEvent: buffer success...");
                                mediaPlayer.play();
                            } else {
                                onLoading();
                                MLog.d("缓冲: " + Math.floor(event.getBuffering()) + "%");
                            }
                            break;

                        case MediaPlayer.Event.Opening:
                            MLog.d("onEvent: opening...");
                            onLoading();
                            break;

                        case MediaPlayer.Event.Playing:
                            MLog.d("onEvent: playing...");
                            break;

                        case MediaPlayer.Event.EndReached:
                            MLog.d("onEvent: completion...");
                            onCompletion(mediaPlayer);
                            break;

                        case MediaPlayer.Event.EncounteredError:
                            MLog.d("onEvent: error...");
                            mediaPlayer.stop();
